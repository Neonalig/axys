<!-- SPDX-License-Identifier: AGPL-3.0-or-later -->

# Axys documentation

| Document                               | What it answers                                                                |
| -------------------------------------- | ------------------------------------------------------------------------------ |
| [design_bible.md](design_bible.md)     | What Axys must do, how editing behaves, and the quality bar. The authority.    |
| [decisions.md](decisions.md)           | Which implementation choices were made, why, and what limitations remain.      |
| [core_contracts.md](core_contracts.md) | The public Rust names, shapes and semantics `crates/axys-core` must implement. |
| [web_contracts.md](web_contracts.md)   | The public TypeScript modules and APIs `web/src` must implement.               |
| [testing.md](testing.md)               | What is checked automatically, what has to be listened to, and the envelope.   |
| [deployment.md](deployment.md)         | How a build is produced and hosted.                                            |

Where two disagree, the design bible wins and the other is the defect.
